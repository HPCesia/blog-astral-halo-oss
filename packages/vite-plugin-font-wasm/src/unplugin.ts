...
class UnionFontPlugin {
  ...
  async start() {
    ...
    if (!this.config.cacheDir) this.config.cacheDir = 'node_modules/.cache/.font';
  ...
  createConfig(config: Options) {
    const scanFiles =
      typeof config.scanFiles === 'object' && !(config.scanFiles instanceof Array)
        ? config.scanFiles
        : {
  ...
  getWatchingPlugin(path: string) {
    const matchedPlugins: SubsetBundlePlugin[] = [];
    this.pluginStore.forEach((plugin, _) => {
...
export const unpluginFactory: UnpluginFactory<Options | undefined> = (config = {}) => {
