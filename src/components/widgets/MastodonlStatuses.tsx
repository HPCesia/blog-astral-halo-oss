...
const mastodonBaseMediaAttachmentSchema = z.object({
  id: z.string(),
  url: z.string(),
  description: z.string().nullable(),
});
const mastodonImageVideoAttachmentSchema = mastodonBaseMediaAttachmentSchema.extend({
...
const mastodonAudioAttachmentSchema = mastodonBaseMediaAttachmentSchema.extend({
  type: z.literal('audio'),
  preview_url: z.null(),
  blurhash: z.null(),
});
const mastodonMediaAttachmentSchema = z.union([
  mastodonImageVideoAttachmentSchema,
  mastodonAudioAttachmentSchema,
]);
...
const mastodonEmojiSchema = z.object({
  shortcode: z.string(),
  url: z.string(),
});
const mastodonStatusSchema = z.object({
  ...
  emojis: z.array(mastodonEmojiSchema),
  media_attachments: z.array(mastodonMediaAttachmentSchema),
  reblog: z.object().nullable(),
  account: mastodonAccountSchema,
});
type MastodonStatus = z.infer<typeof mastodonStatusSchema>;

const CACHE_KEY = 'mastodon-statuses-cache';
const CACHE_EXPIRE = 30 * 60 * 1000;
const getFromCache = (): MastodonStatus[] | null => {
  try {
...
const saveToCache = (data: MastodonStatus[]) => {
  try {
...
const MastodonStatuses: Component<Props> = (props) => {
  const [statuses, setStatuses] = createSignal<MastodonStatus[]>([]);
  ...
  const loadStatuses = async () => {
    ...
    try {
      ...
      const rawData: MastodonStatus[] = await res.json();
      const list = rawData
        .map((s) => {
          const parsed = mastodonStatusSchema.safeParse(s);
          if (!parsed.success) {
              ...
              <li class="border border-base-300 bg-base-200 rounded-box overflow-hidden">
                <div class="p-4 grid grid-cols-1 gap-4">
                  ...
                  <a href={status.account.url} class="flex items-center gap-2">
                    <img src={status.account.avatar} class="avatar w-10 rounded-full" />
                              ...
                              <Show when={attachment.type === 'image'}>
                                {(_) => {
                                  const image = attachment as z.infer<
                                    typeof mastodonImageVideoAttachmentSchema
                                  >; // fix type error
                                  return (
                                    <a
...
export default MastodonStatuses;
